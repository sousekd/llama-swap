...
describe("model loading", () => {
  it("accepts an upstream 404 after the model loads", async () => {
    ...
    await expect(loadModel("b")).resolves.toBeUndefined();
    expect(consoleError).not.toHaveBeenCalled();
    consoleError.mockRestore();
  });

  it("accepts a non-JSON upstream 404 after the model loads", async () => {
    const consoleError = vi.spyOn(console, "error").mockImplementation(() => {});
    vi.stubGlobal(
      "fetch",
      vi.fn().mockResolvedValue({
  ...
  it("surfaces a llama-swap 404", async () => {
    vi.stubGlobal(
      "fetch",
      vi.fn().mockResolvedValue({
        ok: false,
    ...
    await expect(loadModel("b")).rejects.toThrow("Failed to load model: model not found");
  });

  it("surfaces an unmarked non-404 upstream error", async () => {
    vi.stubGlobal(
      "fetch",
      vi.fn().mockResolvedValue({
        ok: false,
    ...
    await expect(loadModel("b")).rejects.toThrow("Failed to load model: upstream failed");
  });
