...
func TestServer_ProcessStreamingResponse_ObservedRates(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "TabbyAPI rates win", body: stream(`data: {"object":"chat.completion.chunk","choices":[],"usage":{"prompt_tokens":10,"completion_tokens":5,"prompt_tokens_per_sec":35,"completion_tokens_per_sec":17}}`), observer: observed, wantPrompt: 35, wantOutput: 17},
